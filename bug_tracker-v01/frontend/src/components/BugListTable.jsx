...
const BugListTable = ({ tableData }) => {
  const getStatusBadgeColor = (status) => {
    switch (status) {
      case 'Closed': return 'bg-green-100 text-green-500 border border-green-200';
      case 'Open': return 'bg-red-100 text-red-500 border border-red-200';
      case 'In Progress': return 'bg-yellow-100 text-yellow-500 border border-yellow-200';
  ...
  const getSeverityBadgeColor = (severity) => {
    switch (severity) {
      ...
      case 'Minor': return 'bg-blue-100 text-blue-500 border border-blue-200';
      default: return 'bg-gray-100 text-gray-500 border border-gray-200';
    }
  }
  ...
  return (
    <div className='overflow-x-auto p-0 rounded-lg mt-3'>
      <table className='min-w-full'>
        ...
        <tbody>
          {(tableData || []).map((bug) => (
            <tr key={bug._id} className='border-t border-gray-200'>
              <td className='py-3 px-4 text-gray-700 text-[13px] line-clamp-1 overflow-hidden'>{bug.title}</td>
