...
const MyBugs = () => {
  ...
  const fetchMyBugs = async () => {
    setLoading(true);
    try {
      console.log('Fetching bugs for user:', user._id);
      
      // Try the dedicated user bugs endpoint
      let response;
      try {
        response = await axiosInstance.get(API_PATHS.BUGS.GET_USER_BUGS(user._id));
        ...
        const sortedBugs = bugsData.sort((a, b) => {
        ...
        console.log('Falling back to GET_ALL_BUGS and filtering client-side');
        // Fallback: fetch all bugs and filter by createdBy
        const allBugsResponse = await axiosInstance.get(API_PATHS.BUGS.GET_ALL_BUGS);
        // Filter bugs where createdBy matches current user ID
        const userBugs = allBugsResponse.data.bugs.filter(bug => 
          bug.createdBy === user._id || 
          (bug.createdBy && bug.createdBy._id === user._id)
        );
        
        // Sort bugs by creation date (newest first)
        const sortedBugs = userBugs.sort((a, b) => {
          return new Date(b.createdAt) - new Date(a.createdAt);
        });
    ...
    <DashboardLayout activeMenu="My Reported Bugs">
      <div className="container mx-auto px-4 py-6">
        ...
        <BugStatusTabs
          ...
          tabs={[
            { label: 'All', count: bugs.length },
            { label: 'Open', count: bugs.filter(bug => bug.status === 'Open').length },
            { label: 'In Progress', count: bugs.filter(bug => bug.status === 'In Progress').length },
            { label: 'Closed', count: bugs.filter(bug => bug.status === 'Closed').length }
          ]}
        />
