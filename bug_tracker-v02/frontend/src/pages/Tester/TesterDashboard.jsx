...
const TesterDashboard = () => {
  ...
  const getDashboardStats = async () => {
    try {
      ...
      console.log('Tester dashboard response:', response.data);
  ...
  const getUserBugs = async () => {
    try {
      ...
      console.log('Fetching user bugs directly for user:', user._id);
      const response = await axiosInstance.get(API_PATHS.BUGS.GET_ALL_VIEWABLE_BUGS);
      
      if (response.data && Array.isArray(response.data.bugs)) {
        // Sort by update date (newest first)
        const sortedBugs = response.data.bugs.sort((a, b) => {
          return new Date(b.updatedAt) - new Date(a.updatedAt);
        });
        
        // Take only the most recent bugs for display
        const mostRecentBugs = sortedBugs.slice(0, 5);
        console.log('Most recent user bugs:', mostRecentBugs);
        
        setRecentBugs(mostRecentBugs);
        return true;
      } else {
        console.error('Invalid response format for user bugs:', response.data);
        return false;
      }
    } catch (error) {
    ...
    <DashboardLayout activeMenu="Dashboard">
      <div className="card my-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-3 md:gap-6 mt-5">
          ...
          <InfoCard
            label="Open Bugs"
            value={addThousandsSeparator(dashboardData?.charts?.bugDistribution?.Open || 0)}
            color="bg-violet-500"
          />
          <InfoCard
          ...
          <InfoCard
            label="Closed Bugs"
            value={addThousandsSeparator(dashboardData?.charts?.bugDistribution?.Closed || 0)}
            color="bg-lime-500"
          />
        </div>
          ...
          <div className="card">
            ...
            {loading ? (
              ...
              <div className="text-center py-12">
                <p className="text-gray-500 mb-4">You haven't reported any bugs yet.</p>
                <button
